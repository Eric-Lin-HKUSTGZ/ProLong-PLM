...
class Trainer(HFTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, return_output_and_metrics=False):
        ...
        try:
            # train PLM setting
            allowed_keys = ["input_ids", "attention_mask", "position_ids", "past_key_values", "inputs_embeds","labels", "use_cache", "output_attentions", "output_hidden_states", "return_dict", "cache_position"]
            filtered_inputs = {k: v for k, v in inputs.items() if k in allowed_keys}
            outputs = model(**filtered_inputs, use_cache=False)
            
            # train ProLong setting
