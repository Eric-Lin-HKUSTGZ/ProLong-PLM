...
class SortByLengthDataset(StreamingDataset):
    def __init__(self, *args, sort_by_length_size=1, data_args=None, download_timeout=600, **kwargs):
        super().__init__(*args, download_timeout=download_timeout, **kwargs)
...
def build_dataset(paths, training_args: TrainingArguments, data_args: DataArguments, is_training: bool, download_timeout: int = 600) -> StreamingDataset:
    ...
    for path in paths:
        ...
        # 记录index.json路径
        index_files.append(os.path.join(path, "index.json"))

    # 等待index.json文件出现（仅非rank0进程）
    ...
    if rank != 0:
        ...
        for index_file in index_files:
            logger.info(f"[RANK {rank}] Waiting for index file: {index_file}")
            waited = 0
            while not os.path.exists(index_file) and waited < download_timeout:
                time.sleep(2)
                waited += 2
            if not os.path.exists(index_file):
                raise RuntimeError(f"[RANK {rank}] Timeout waiting for index file: {index_file}")
            logger.info(f"[RANK {rank}] Index file found: {index_file}")
