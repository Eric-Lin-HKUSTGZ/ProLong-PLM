...
# @torch.jit.script
# RMSNorm归一化函数，等价于T5LayerNorm
# 输入：hidden_states（张量），weight（可学习参数），variance_epsilon（数值稳定性）
# 输出：归一化后的hidden_states
# 用于LlamaRMSNorm类
...
class LlamaRMSNorm(nn.Module):
    """
    LlamaRMSNorm: LLaMA模型的RMS归一化层，等价于T5LayerNorm。
    参数：
        hidden_size: 隐藏层维度
        eps: 数值稳定性参数
    成员变量：
        weight: 可学习缩放参数
        variance_epsilon: 数值稳定性
    前向输出：归一化后的hidden_states
    """
...
class LlamaRotaryEmbedding(nn.Module):
    """
    LlamaRotaryEmbedding: 实现RoPE旋转位置编码，支持多种类型和动态长度。
    参数：
        dim, max_position_embeddings, base, scaling_factor, rope_type, interleaved, config
    主要成员：
        rope_init_fn: 位置编码初始化函数
        inv_freq: 旋转频率
        attention_scaling: 缩放因子
        _cos_cached/_sin_cached: 缓存的cos/sin表
    forward输入：q, k（query/key），seqlen_offset，unpadded_lengths
    输出：旋转位置编码后的q, k
    """
    def __init__(
        self,
    ...
    def forward(
        ...
        seqlen_offset: int = 0, # 用于序列并行，每个设备只看一部分序列
...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    """
    repeat_kv: 用于多头注意力中key/value的复制扩展
    输入：hidden_states (B, S, H, D), n_rep
    输出：扩展后的hidden_states
    """
    if n_rep == 1:
...
class LlamaAttention(nn.Module):
    """
    LlamaAttention: 多头自注意力模块，支持FlashAttention和分布式注意力。
    参数：config, layer_idx, context_window_toggle
    主要成员：
        q_proj, k_proj, v_proj, o_proj: 线性变换
        rotary_emb: 旋转位置编码
        distributed_attn_func: 分布式注意力函数
    forward输入：hidden_states, attention_mask, position_ids, past_key_value, ...
    输出：attn_output, attn_weights, past_key_value
    """
    def __init__(
        self,
        config: LlamaConfig,
    ...
    def forward(
        ...
        **kwargs,
        ...
        前向传播：
        输入：hidden_states等
        输出：attn_output（注意力输出），attn_weights（可选），past_key_value（可选）
        关键流程：QKV投影、RoPE编码、FlashAttention/分布式注意力、缓存KV
        """
...
class LlamaDecoderLayer(nn.Module):
    """
    LlamaDecoderLayer: LLaMA解码器单层，包含自注意力、MLP、归一化等
    参数：config
    成员变量：
        self_attn: 自注意力层
        mlp: 前馈网络
        input_layernorm, post_attention_layernorm: 两个归一化层
        _fsdp_wrap: FSDP分布式训练标记
    forward输入：hidden_states等
    输出：hidden_states, (可选)attn_weights, (可选)present_key_value
    """
    ...
    def forward(
        ...
        输入：hidden_states等
        输出：hidden_states, (可选)attn_weights, (可选)present_key_value
        结构：LayerNorm->自注意力->残差->LayerNorm->MLP->残差
        """
...
class LlamaPreTrainedModel(PreTrainedModel):
    """
    LlamaPreTrainedModel: 预训练模型基类，提供权重初始化等通用功能
    """
...
class LlamaModel(LlamaPreTrainedModel):
    """
    LlamaModel: LLaMA主干Transformer模型，由多层LlamaDecoderLayer堆叠组成。
    参数：config
    成员变量：
        embed_tokens: 词嵌入层
        layers: 解码器层列表
        norm: 输出归一化层
    forward输入：input_ids, attention_mask, position_ids, ...
    输出：last_hidden_state, (可选)past_key_values, (可选)hidden_states, (可选)attentions
    """
    def __init__(self, config: LlamaConfig):
    ...
    def forward(
        ...
        if input_ids is not None:
            print(f"--- [PLMForCausalLM.forward] input_ids.shape: {input_ids.shape} ---")
            print(f"--- [PLMForCausalLM.forward] input_ids: {input_ids} ---")
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    """
    LlamaForCausalLM: 用于自回归语言建模的顶层封装，输出logits和loss。
    参数：config
    成员变量：
        model: LlamaModel主干
        lm_head: 输出层
        logit_block_size: 分块计算logits（节省显存）
    forward输入：input_ids, attention_mask, labels等
    输出：loss, logits, (可选)past_key_values, (可选)hidden_states, (可选)attentions
    关键方法：compute_loss（支持token_scaled_loss）、prepare_inputs_for_generation
    """
    ...
    def compute_loss(self, hidden_states, labels, token_losses=False):
        """
        计算交叉熵损失，支持token_scaled_loss（按有效token数缩放loss）
        输入：hidden_states, labels
        输出：loss
        """
    ...
    def prepare_inputs_for_generation(
        ...
        """
        用于生成任务的输入准备，兼容past_key_values和inputs_embeds
        """
        if past_key_values:
...
class LlamaForSequenceClassification(LlamaPreTrainedModel):
    """
    LlamaForSequenceClassification: 用于序列分类任务的顶层封装。
    参数：config
    成员变量：
        model: LlamaModel主干
        score: 分类输出层
